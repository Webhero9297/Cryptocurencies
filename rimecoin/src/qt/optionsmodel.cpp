...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RIM);
    ...
    if (!settings.contains("nAnonymizeRimAmount"))
        settings.setValue("nAnonymizeRimAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeRimAmount = settings.value("nAnonymizeRimAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRimAmount"))
        SoftSetArg("-anonymizerimamount", settings.value("nAnonymizeRimAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rim.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRimAmount:
            return QVariant(nAnonymizeRimAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeRimAmount:
            nAnonymizeRimAmount = value.toInt();
            settings.setValue("nAnonymizeRimAmount", nAnonymizeRimAmount);
            emit anonymizeRimAmountChanged(nAnonymizeRimAmount);
            break;
        case CoinControlFeatures:
