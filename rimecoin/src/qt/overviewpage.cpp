...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::RIM)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("RIM")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeRimAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRimAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeRimAmount = strAnonymizeRimAmount.remove(strAnonymizeRimAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRimAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeRimAmount * COIN) nMaxToAnonymize = nAnonymizeRimAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeRimAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeRimAmount));
        strAnonymizeRimAmount = strAnonymizeRimAmount.remove(strAnonymizeRimAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRimAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
